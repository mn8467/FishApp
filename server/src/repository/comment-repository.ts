import pool from "../db";
import { CommentCreateDTO, CommentDTO, ResponseCommentDTO, UserWriteCommentDTO } from "../dto/comment-dto";
import { DbError } from "../utils/error";  

export async function findCommentByFishIdForUserWrited(fish_id:number, user_id:number): Promise<UserWriteCommentDTO[]>{
  const sql = `
                    SELECT
                      ...
                      COALESCE(cnt.cnt, 0)         AS "likeCount",
                    ...
                    LEFT JOIN (
                      ...
                      ON cnt.comment_id = c.comment_id
              ...
              `;  
 try {
    const { rows } = await pool.query(sql, [fish_id, user_id]);
    return rows;
  } catch (e) {
    throw new DbError("Failed to load comments", e);
  }
}
